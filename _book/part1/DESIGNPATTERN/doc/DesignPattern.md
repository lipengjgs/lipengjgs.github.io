# [设计模式](https://github.com/addyosmani/essential-js-design-patterns)

1. [介绍](#介绍)
   1. [1. GOF(Gang of Four)](#1-gofgang-of-four)
2. [一、分类](#一分类)
   1. [1. 创作设计模式(创建型模式Creational Design Patterns)](#1-创作设计模式创建型模式creational-design-patterns)
   2. [2. 结构设计模式(结构型模式Structural Design Patterns)](#2-结构设计模式结构型模式structural-design-patterns)
   3. [3. 行为设计模式(行为型模式Behavioral Design Patterns)](#3-行为设计模式行为型模式behavioral-design-patterns)
   4. [4. J2EE设计模式（特别关注表示层）](#4-j2ee设计模式特别关注表示层)
3. [二、详述](#二详述)
   1. [1. 适配器模式](#1-适配器模式)
   2. [2. 建造者模式](#2-建造者模式)
   3. [3. 工厂模式](#3-工厂模式)
   4. [4. 抽象工厂模式](#4-抽象工厂模式)
   5. [1. JavaScript设计模式](#1-javascript设计模式)
   6. [2. JavaScript MV* 模式](#2-javascript-mv-模式)
   7. [3. 现代模块化设计模式](#3-现代模块化设计模式)
   8. [4. JavaScript命名空间模式](#4-javascript命名空间模式)
   9. [5. 插件设计模式](#5-插件设计模式)
      1. [一、单例模式](#一单例模式)
      2. [二、二段构建模式](#二二段构建模式)
      3. [三、管理者模式](#三管理者模式)
      ...
      9. [九、委托模式](#九委托模式)
      10. [练习](#练习)
   10. [工厂模式（Factory Pattern）](#工厂模式factory-pattern)
   11. [抽象工厂模式（Abstract Factory Pattern）](#抽象工厂模式abstract-factory-pattern)
   12. [单例模式（Singleton Pattern）](#单例模式singleton-pattern)
   13. [原型模式（Prototype Pattern）](#原型模式prototype-pattern)
   14. [桥接（Bridge）](#桥接bridge)
   15. [过滤器模式（Filter Pattern）或标准模式（Criteria Pattern）](#过滤器模式filter-pattern或标准模式criteria-pattern)
   16. [组合模式（Composite Pattern）](#组合模式composite-pattern)
   17. [装饰器模式（Decorator Pattern）](#装饰器模式decorator-pattern)
   18. [外观模式（Facade Pattern）](#外观模式facade-pattern)
   19. [享元模式（Flyweight Pattern）](#享元模式flyweight-pattern)
   20. [代理模式（Proxy Pattern）](#代理模式proxy-pattern)
   21. [责任链模式（Chain of Responsibility Pattern）](#责任链模式chain-of-responsibility-pattern)
   22. [命令模式（Command Pattern）](#命令模式command-pattern)
   23. [解释器模式（Interpreter Pattern）](#解释器模式interpreter-pattern)
   24. [迭代器模式（Iterator Pattern）](#迭代器模式iterator-pattern)
   25. [中介者模式（Mediator Pattern）](#中介者模式mediator-pattern)
   26. [备忘录模式（Memento Pattern）](#备忘录模式memento-pattern)
   27. [当对象间存在一对多关系时，则使用观察者模式（Observer Pattern）](#当对象间存在一对多关系时则使用观察者模式observer-pattern)
   28. [状态模式（State Pattern）](#状态模式state-pattern)
   29. [空对象模式（Null Object Pattern）](#空对象模式null-object-pattern)
   30. [策略模式（Strategy Pattern）](#策略模式strategy-pattern)
   31. [模板模式（Template Pattern）](#模板模式template-pattern)
   32. [访问者模式（Visitor Pattern）](#访问者模式visitor-pattern)
   33. [MVC 模式](#mvc-模式)
   34. [业务代表模式（Business Delegate Pattern）](#业务代表模式business-delegate-pattern)
   35. [组合实体模式（Composite Entity Pattern）](#组合实体模式composite-entity-pattern)
   36. [数据访问对象模式（Data Access Object Pattern）或 DAO 模式](#数据访问对象模式data-access-object-pattern或-dao-模式)
   37. [前端控制器模式（Front Controller Pattern）](#前端控制器模式front-controller-pattern)
   38. [拦截过滤器模式（Intercepting Filter Pattern）](#拦截过滤器模式intercepting-filter-pattern)
   39. [服务定位器模式（Service Locator Pattern）](#服务定位器模式service-locator-pattern)
   40. [传输对象模式（Transfer Object Pattern）](#传输对象模式transfer-object-pattern)
4. [设计模式相关的网站](#设计模式相关的网站)
...
### 1. 创作设计模式(创建型模式Creational Design Patterns)

> 解决哪一类问题：对象创建（在创建对象的同时隐藏创建逻辑的方式）
...
### 2. 结构设计模式(结构型模式Structural Design Patterns)

> 解决哪一类问题：对象结构（类和对象的组合）
...
### 3. 行为设计模式(行为型模式Behavioral Design Patterns)

> 解决哪一类问题：独立对象之间相互沟通（对象之间的通信）
...
- Interpreter - 针对`class`
- TemplateMethod - 针对`class`
- Iterator
- Mediator
- Observer
- Visitor
...
> 作为两个不兼容接口之间的桥梁, 像现实生活中的读卡器：连接内存卡与电脑，读卡器作为单独的一个类而存在
...
- 注意事项：适配器不是在详细设计时添加的，而是解决**正在服役的项目的问题**。
...
```javascript
var newO = {};
var newO = Object.create(Object.prototype);
var newO = new Object();
```
...
```javascript
// Way 01
newO.someKey = 'HelloWorld';
var val = newO.someKey;
...
// Way 03
Object.defineProperty(newO, "someKey", {
...
defineProp(person, 'car', 'dazhong');
defineProp(person, 'birth', '1991');
defineProp(person, 'hasBeard', true);
console.log(person)

// Way 04
Object.defineProperties(newO, {
    'someKey': {
        value: 'Hello',
        writable: true,
        enumerable: true,
...
- Singleton
  > 有两点需求： 假如没有则创建的实例，假如有则指向已创建的实例，JS中单例提供一个公共的资源空间，并且只提供单一个方法入口
  ...
  ```javascript
    const mySingleton = (function() {
        ...
        function init() {
            // Private
            let privateVariable = 'I am privateVariable';
            let privateFunction = () => {
                console.log('I am privateFunction been called in public func')
            }
...
[练习](https://codepen.io/lipengjgs/pen/jOWqzyK)

[UML](http://c.biancheng.net/view/1319.html)

> UML

又称统一建模语言或标准建模语言，是始于1997年一个OMG标准，它是一个支持模型化和软件系统开发的图形化语言，为软件开发的所有阶段提供模型化和可视化支持，包括由需求分析到规格，到构造和配置。 面向对象的分析与设计(OOA&D，OOAD)方法的发展在80年代末至90年代中出现了一个高潮，UML是这个高潮的产物。它不仅统一了Booch、Rumbaugh和Jacobson的表示方法，而且对其作了进一步的发展，并最终统一为大众所接受的标准建模语言。

从目标系统的不同角度出发，定义了用例图、类图(类名；属性；操作)、对象图、状态图、活动图、时序图、协作图、构件图、部署图等 9 种图
...
import java.util.List;public interface Criteria {
...
import java.util.ArrayList;import java.util.List;public class CriteriaMale implements Criteria {
   @Override
   public List<Person> meetCriteria(List<Person> persons) {
...
import java.util.ArrayList;import java.util.List;public class CriteriaFemale implements Criteria {
   @Override
   public List<Person> meetCriteria(List<Person> persons) {
...
import java.util.ArrayList;import java.util.List;public class CriteriaSingle implements Criteria {
   @Override
   public List<Person> meetCriteria(List<Person> persons) {
...
import java.util.List;public class AndCriteria implements Criteria {
...
import java.util.List;public class OrCriteria implements Criteria {
   private Criteria criteria;
   private Criteria otherCriteria;
...
import java.util.ArrayList;import java.util.List;public class Employee {
   private String name;
   private String dept;
   private int salary;
...
import java.util.HashMap;public class ShapeFactory {
...
import java.util.Date;public class ChatRoom {
...
public class Computer implements ComputerPart {
   ...
   public Computer(){
      parts = new ComputerPart[] {new Mouse(), new Keyboard(), new Monitor()};j
   } 
...
> 用于从客户端向服务器一次性传递带有多个属性的数据。传输对象也被称为数值对象。传输对象是一个具有 getter/setter 方法的简单的 POJO 类，它是可序列化的，所以它可以通过网络传输。它没有任何的行为。服务器端的业务类通常从数据库读取数据，然后填充 POJO，并把它发送到客户端或按值传递它。对于客户端，传输对象是只读的。客户端可以创建自己的传输对象，并把它传递给服务器，以便一次性更新数据库中的数值。以下是这种设计模式的实体。
>
